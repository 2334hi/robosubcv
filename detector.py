...
rospy.init_node('CV_pub', anonymous=True)
pub = rospy.Publisher('CV', Float64MultiArray, queue_size=10)

def det_formating(detections, im_shape):
    bboxes = []
...
def torch_thread(weights, img_size, conf_thres=0.2, iou_thres=0.45):
    ...
    while not exit_signal:
        if run_signal:
            ...
            formated = det_formating(detections)
            formated = formated.flatten().tolist()
